...
/* Suppress MSVC "unsafe function" deprecation warnings for standard C99
 * functions (strncpy, fopen, scanf). These are safe in this codebase and
 * the _s variants are MSVC-only, breaking portability to gcc/clang. */
...
/* CPU BLAS acceleration (optional — pick one at build time).
 *
 *  NML_USE_ACCELERATE  : Apple Accelerate framework (macOS/iOS)
 *  NML_USE_OPENBLAS    : OpenBLAS (Linux/Windows, any x86/ARM)
 *  NML_USE_ONEMKL_CPU  : Intel oneMKL CPU (Intel CPUs; no GPU or SYCL required)
 *                        Build: icx -O3 -DNML_USE_ONEMKL_CPU runtime/nml.c -lm -qmkl=sequential
 *  NML_USE_AOCL        : AMD Optimizing CPU Libraries / AOCL-BLAS (AMD Zen CPUs)
 *                        Build: gcc -O3 -DNML_USE_AOCL runtime/nml.c -lm -lblis-mt
 *
 * All four expose the standard cblas_sgemm / cblas_dgemm interface, so the
 * dispatch code below is shared across all backends.
 */
  ...
  #include <mkl_cblas.h>
  #define NML_HAS_BLAS 1
#elif defined(NML_USE_AOCL)
  #include <cblas.h>   /* AOCL installs a cblas-compatible header */
  #define NML_HAS_BLAS 1
#endif

/* Metal GPU acceleration (optional, macOS/iOS)
   Implementation lives in nml_backend_metal.m (compiled separately as Objective-C).
   Threshold and Tensor type are in nml_tensor.h. */
#ifdef NML_USE_METAL
  extern int nml_backend_metal_matmul(Tensor *dest, const Tensor *a, const Tensor *b,
                                      int m, int k, int n);
#endif

/* Hailo NPU acceleration (optional, Raspberry Pi AI HAT+ / AI HAT+ 2)
   Implementation lives in nml_backend_hailo.cpp (compiled separately, requires libhailort).
   Unlike SYCL/BLAS, Hailo dispatches at whole-program level using pre-compiled HEF files.
   See transpilers/nml_to_hailo.py for offline NML → HEF compilation.
 *
 * Two deployment modes:
 *   Default (NML_USE_HAILO):
 *     HEF lives on disk; located by chip-specific probe at runtime.
 *   Embedded (NML_USE_HAILO + NML_EMBEDDED_HEF):
 *     HEF is compiled into the binary as a C byte array via:
 *       make nml-rpi-hailo-embed PROGRAM=... DATA=... ARCH=...
 *     No file I/O — fully self-contained single-binary deployment.  */
#ifdef NML_USE_HAILO
  #include "nml_backend_hailo.h"
  #ifdef NML_EMBEDDED_HEF
    /* Generated by make nml-rpi-hailo-embed — defines:
     *   static const unsigned char _nml_embedded_hef_data[];
     *   static const unsigned int  _nml_embedded_hef_size;   */
    #include "nml_hef_resource.h"
  #endif
#endif

/* SYCL GPU acceleration (optional, Intel/AMD/NVIDIA via oneAPI or AdaptiveCpp)
   Implementation lives in nml_backend_sycl.cpp (compiled separately as C++ with -fsycl).
   Thresholds and Tensor type are in nml_tensor.h. */
#ifdef NML_USE_SYCL
  extern int nml_backend_sycl_matmul(Tensor *dest, const Tensor *a, const Tensor *b,
                                     int m, int k, int n);
  extern int nml_backend_sycl_relu   (float *data, int n);
  extern int nml_backend_sycl_sigmoid(float *data, int n);
  extern int nml_backend_sycl_tanh   (float *data, int n);
  /* Phase 2 */
  extern int nml_backend_sycl_conv2d(Tensor *out, const Tensor *input, const Tensor *kernel,
                                     int stride, int pad);
  extern int nml_backend_sycl_gelu(float *data, int n);
  extern int nml_backend_sycl_softmax(float *data, int rows, int cols);
  extern int nml_backend_sycl_layernorm(float *data, int rows, int cols,
                                        const float *gamma, const float *beta);
  extern int nml_backend_sycl_attention(Tensor *out, const Tensor *Q,
                                        const Tensor *K, const Tensor *V);
#endif

/* CUDA GPU acceleration (optional, NVIDIA GPUs via cuBLAS)
   Implementation lives in nml_backend_cuda.cu (compiled separately with nvcc).
   Build: gcc -DNML_USE_CUDA -c nml.c && nvcc nml_cuda_core.o nml_backend_cuda.cu -lcublas
   Thresholds: NML_CUDA_MMUL_THRESHOLD (default 4096), NML_CUDA_EW_THRESHOLD (default 4096) */
#ifdef NML_USE_CUDA
  #include "nml_backend_cuda.h"
  extern int nml_backend_cuda_matmul(Tensor *dest, const Tensor *a, const Tensor *b,
                                     int m, int k, int n);
  extern int nml_backend_cuda_relu   (float *data, int n);
  extern int nml_backend_cuda_sigmoid(float *data, int n);
  extern int nml_backend_cuda_tanh   (float *data, int n);
#endif
...
/* Windows (MSVC) compat */
  ...
  static int clock_gettime(int clk, struct timespec *ts) {
      if (clk == CLOCK_MONOTONIC) {
          ...
          FILETIME ft; ULARGE_INTEGER ul;
          GetSystemTimeAsFileTime(&ft);
          ul.LowPart = ft.dwLowDateTime; ul.HighPart = ft.dwHighDateTime;
          ul.QuadPart -= 116444736000000000ULL; /* epoch offset */
          ts->tv_sec  = (time_t)(ul.QuadPart / 10000000ULL);
          ts->tv_nsec = (long)((ul.QuadPart % 10000000ULL) * 100);
      }
      return 0;
  }
#else
...
/* Runtime-overridable GPU dispatch thresholds (defaults from compile-time macros) */
...
#endif
static int _nml_cuda_mmul_thr = NML_CUDA_MMUL_THRESHOLD;
static int _nml_cuda_ew_thr   = NML_CUDA_EW_THRESHOLD;
static int _nml_sycl_mmul_thr = NML_SYCL_MMUL_THRESHOLD;
static int _nml_sycl_ew_thr   = NML_SYCL_EW_THRESHOLD;

static void nml_gpu_thresholds_init(void) {
    const char *v;
    if ((v = getenv("NML_CUDA_MMUL_THRESHOLD")) != NULL) _nml_cuda_mmul_thr = atoi(v);
    if ((v = getenv("NML_CUDA_EW_THRESHOLD"))   != NULL) _nml_cuda_ew_thr   = atoi(v);
    if ((v = getenv("NML_SYCL_MMUL_THRESHOLD")) != NULL) _nml_sycl_mmul_thr = atoi(v);
    if ((v = getenv("NML_SYCL_EW_THRESHOLD"))   != NULL) _nml_sycl_ew_thr   = atoi(v);
}
...
/* DType, Tensor, error codes, NML_MAX_*, and backend thresholds are in nml_tensor.h */
...
/* Tensor struct definition is in nml_tensor.h */
...
static int tensor_init_typed(Tensor *t, int ndim, const int *shape, DType dtype) {
    ...
    for (int i = 0; i < ndim; i++) {
        if (shape[i] <= 0) return NML_ERR_OVERFLOW;
        if (new_size > NML_MAX_TENSOR_SIZE / shape[i]) return NML_ERR_OVERFLOW;
        new_size *= shape[i];
    }
...
/* Dispatch order: SYCL → CUDA → Metal → BLAS → CPU fallback.
 * GPU backends only handle F32; F64/I32 workloads fall through to BLAS/CPU. */
#ifdef NML_USE_SYCL
    if (m * n >= _nml_sycl_mmul_thr &&
        a->dtype == NML_F32 && b->dtype == NML_F32 &&
        nml_backend_sycl_matmul(dest, a, b, m, k1, n) == 0) {
...
#ifdef NML_USE_CUDA
    if (m * n >= _nml_cuda_mmul_thr &&
        a->dtype == NML_F32 && b->dtype == NML_F32 &&
        nml_backend_cuda_matmul(dest, a, b, m, k1, n) == 0) {
        if (dest == &tmp) { tensor_free(out); *out = tmp; }
        return NML_OK;
    }
#endif
...
#ifdef NML_USE_METAL
    if (m * n >= NML_METAL_MMUL_THRESHOLD &&
        a->dtype == NML_F32 && b->dtype == NML_F32 &&
        nml_backend_metal_matmul(dest, a, b, m, k1, n) == 0) {
        if (dest == &tmp) { tensor_free(out); *out = tmp; }
        return NML_OK;
    }
#endif
...
/* Compute b's flat index given a's flat index, using broadcast strides.
 * b_stride[d] == 0 means broadcast in that dim (b has size 1 there). */
static int tensor_bcast_idx(int flat_i, const int *a_shape, int ndim, const int *b_stride) {
    int b_idx = 0;
    for (int d = ndim - 1; d >= 0; d--) {
        int coord = flat_i % a_shape[d];
        flat_i /= a_shape[d];
        b_idx += coord * b_stride[d];
    }
    return b_idx;
}

/* Check if b can broadcast to match a's shape (NumPy right-align rules).
 * Fills b_stride[d]: actual stride for matching dims, 0 for broadcast dims.
 * Returns 0 on success, NML_ERR_SHAPE on incompatible shapes. */
static int tensor_bcast_check(const Tensor *a, const Tensor *b, int *b_stride) {
    int ndim = a->ndim;
    int b_off = ndim - b->ndim;
    for (int d = 0; d < ndim; d++) {
        int b_d = d - b_off;
        int b_dim = (b_d >= 0) ? b->shape[b_d] : 1;
        if (b_dim == 1) {
            b_stride[d] = 0;
        } else if (b_dim == a->shape[d]) {
            int stride = 1;
            int b_start = (b_d >= 0) ? b_d + 1 : 0;
            for (int k = b_start; k < b->ndim; k++) stride *= b->shape[k];
            b_stride[d] = stride;
        } else {
            return NML_ERR_SHAPE;
        }
    }
    return NML_OK;
}
...
static void tensor_add(Tensor *out, const Tensor *a, const Tensor *b) {
    ...
    Tensor tmp = {0};
    Tensor *dest = (out == a || out == b) ? &tmp : out;
    tensor_init_typed(dest, a->ndim, shape, dt);
    if (a->size == b->size) {
        ...
        /* broadcast path */
        int b_stride[NML_MAX_DIMS] = {0};
        if (tensor_bcast_check(a, b, b_stride) != 0) {
            /* legacy fallback: row broadcasting (b evenly divides a) */
            int bmod = (b->size > 0 && a->size % b->size == 0) ? b->size : a->size;
            for (int i = 0; i < a->size; i++)
                tensor_setd(dest, i, tensor_getd(a, i) + tensor_getd(b, i % bmod));
        } else {
            for (int i = 0; i < a->size; i++)
                tensor_setd(dest, i, tensor_getd(a, i) + tensor_getd(b, tensor_bcast_idx(i, a->shape, a->ndim, b_stride)));
        }
    }
    if (dest == &tmp) { tensor_free(out); *out = tmp; }
}
...
static int tensor_ediv(Tensor *out, const Tensor *a, const Tensor *b) {
    ...
    if (a->size == b->size) {
        ...
        if (tensor_bcast_check(a, b, b_stride) != 0) {
            ...
            for (int i = 0; i < a->size; i++) {
                int bi = i % bmod;
                if (tensor_getd(b, bi) == 0.0) { if (dest == &tmp) tensor_free(&tmp); return NML_ERR_DIVZERO; }
                tensor_setd(dest, i, tensor_getd(a, i) / tensor_getd(b, bi));
            }
        } else {
            for (int i = 0; i < a->size; i++) {
                int bi = tensor_bcast_idx(i, a->shape, a->ndim, b_stride);
                if (tensor_getd(b, bi) == 0.0) { if (dest == &tmp) tensor_free(&tmp); return NML_ERR_DIVZERO; }
                tensor_setd(dest, i, tensor_getd(a, i) / tensor_getd(b, bi));
            }
        }
...
static void tensor_scale_add(Tensor *out, const Tensor *a, double s) {
    tensor_copy(out, a);
    for (int i = 0; i < a->size; i++) tensor_setd(out, i, tensor_getd(a, i) + s);
}

static void tensor_scale_sub(Tensor *out, const Tensor *a, double s) {
    tensor_copy(out, a);
    for (int i = 0; i < a->size; i++) tensor_setd(out, i, tensor_getd(a, i) - s);
}
...
static void tensor_relu(Tensor *out, const Tensor *t) {
    tensor_copy(out, t);
#ifdef NML_USE_SYCL
    if (out->dtype == NML_F32 && out->size >= _nml_sycl_ew_thr &&
        nml_backend_sycl_relu(out->data.f32, out->size) == 0) return;
#endif
#ifdef NML_USE_CUDA
    if (out->dtype == NML_F32 && out->size >= _nml_cuda_ew_thr &&
        nml_backend_cuda_relu(out->data.f32, out->size) == 0) return;
#endif
...
#ifdef NML_USE_SYCL
    if (out->dtype == NML_F32 && out->size >= _nml_sycl_ew_thr &&
        nml_backend_sycl_sigmoid(out->data.f32, out->size) == 0) return;
#endif
#ifdef NML_USE_CUDA
    if (out->dtype == NML_F32 && out->size >= _nml_cuda_ew_thr &&
        nml_backend_cuda_sigmoid(out->data.f32, out->size) == 0) return;
#endif
    for (int i = 0; i < t->size; i++) {
        double v = tensor_getd(t, i);
...
#ifdef NML_USE_SYCL
    if (out->dtype == NML_F32 && out->size >= _nml_sycl_ew_thr &&
        nml_backend_sycl_tanh(out->data.f32, out->size) == 0) return;
#endif
#ifdef NML_USE_CUDA
    if (out->dtype == NML_F32 && out->size >= _nml_cuda_ew_thr &&
        nml_backend_cuda_tanh(out->data.f32, out->size) == 0) return;
#endif
...
static void tensor_softmax(Tensor *out, const Tensor *t) {
    tensor_copy(out, t);
#ifdef NML_USE_SYCL
    if (t->dtype == NML_F32 && t->size >= _nml_sycl_ew_thr) {
        int rows = (t->ndim >= 2) ? t->shape[0] : 1;
        int cols = t->size / rows;
        if (nml_backend_sycl_softmax(out->data.f32, rows, cols) == 0) return;
    }
#endif
#ifdef NML_USE_CUDA
    if (t->dtype == NML_F32 && t->size >= _nml_cuda_ew_thr) {
        int rows = (t->ndim >= 2) ? t->shape[0] : 1;
        int cols = t->size / rows;
        if (nml_backend_cuda_softmax(out->data.f32, rows, cols) == 0) return;
    }
#endif
...
static void tensor_conv2d(Tensor *out, const Tensor *input, const Tensor *kernel, int stride, int pad) {
    ...
    if (input->ndim == 4) {
        ...
        /* kernel->shape[1] should == C_in */
        KH = kernel->shape[2]; KW = kernel->shape[3];
        OH = (H + 2*pad - KH) / stride + 1;
        OW = (W + 2*pad - KW) / stride + 1;
#ifdef NML_USE_SYCL
        if (input->dtype == NML_F32 && (size_t)(N*C_out*OH*OW) >= (size_t)_nml_sycl_mmul_thr) {
            if (nml_backend_sycl_conv2d(out, input, kernel, stride, pad) == 0) return;
        }
#endif
#ifdef NML_USE_CUDA
        if (input->dtype == NML_F32 && (size_t)(N*C_out*OH*OW) >= (size_t)_nml_cuda_mmul_thr) {
            if (nml_backend_cuda_conv2d(out, input, kernel, stride, pad) == 0) return;
        }
#endif
        int out_shape[] = {N, C_out, OH, OW};
        DType dt = dtype_promote(input->dtype, kernel->dtype);
        tensor_init_typed(out, 4, out_shape, dt);
        for (int n = 0; n < N; n++)
                            ...
                            for (int kh = 0; kh < KH; kh++)
                                for (int kw = 0; kw < KW; kw++) {
                                    ...
                                    if (ih >= 0 && ih < H && iw >= 0 && iw < W) {
                                        int i_idx = n*(C_in*H*W) + ci*(H*W) + ih*W + iw;
                                        int k_idx = co*(C_in*KH*KW) + ci*(KH*KW) + kh*KW + kw;
                                        sum += tensor_getd(input, i_idx) * tensor_getd(kernel, k_idx);
                                    }
                                }
                        int o_idx = n*(C_out*OH*OW) + co*(OH*OW) + oh*OW + ow;
                        tensor_setd(out, o_idx, sum);
                    }
        return;
    }
...
static void tensor_maxpool(Tensor *out, const Tensor *input, int pool_size, int stride) {
    if (input->ndim == 4) {
        ...
        int out_shape[] = {N, C, OH, OW};
        tensor_init_typed(out, 4, out_shape, input->dtype);
        for (int n = 0; n < N; n++)
            for (int c = 0; c < C; c++)
                for (int oh = 0; oh < OH; oh++)
                    for (int ow = 0; ow < OW; ow++) {
                        ...
                        for (int ph = 0; ph < pool_size; ph++)
                            for (int pw = 0; pw < pool_size; pw++) {
                                ...
                                if (ih < H && iw < W) {
                                    double v = tensor_getd(input, n*(C*H*W) + c*(H*W) + ih*W + iw);
                                    if (v > mx) mx = v;
                                }
                            }
                        tensor_setd(out, n*(C*OH*OW) + c*(OH*OW) + oh*OW + ow, mx);
                    }
        return;
    }
            ...
            for (int c = 0; c < C; c++)
                for (int oh = 0; oh < OH; oh++)
                    for (int ow = 0; ow < OW; ow++) {
                        ...
                        tensor_setd(out, n*(C*OH*OW) + c*(OH*OW) + oh*OW + ow,
                                    tensor_getd(input, n*(C*H*W) + c*(H*W) + ih*W + iw));
                    }
        return;
    }
    int H = input->shape[0], W = input->shape[1];
    ...
    if (input->ndim == 4) {
        ...
        for (int n = 0; n < N; n++)
            for (int c = 0; c < C; c++)
                for (int h = 0; h < H; h++)
                    for (int w = 0; w < W; w++)
                        tensor_setd(out, n*(C*OH*OW) + c*(OH*OW) + (h+pad)*OW + (w+pad),
                                    tensor_getd(input, n*(C*H*W) + c*(H*W) + h*W + w));
        return;
    }
    int H = input->shape[0], W = input->shape[1];
    int OH = H + 2*pad, OW = W + 2*pad;
...
#ifdef NML_USE_SYCL
    if (Q->dtype == NML_F32 && (size_t)((long long)seq_len*seq_len) >= (size_t)_nml_sycl_mmul_thr) {
        if (nml_backend_sycl_attention(out, Q, K, V) == 0) return;
    }
#endif
#ifdef NML_USE_CUDA
    if (Q->dtype == NML_F32 && (size_t)((long long)seq_len*seq_len) >= (size_t)_nml_cuda_mmul_thr) {
        if (nml_backend_cuda_attention(out, Q, K, V) == 0) return;
    }
#endif
...
#ifdef NML_USE_SYCL
    if (input->dtype == NML_F32 && input->size >= _nml_sycl_ew_thr) {
        const float *gp = (gamma && gamma->dtype == NML_F32) ? gamma->data.f32 : NULL;
        const float *bp = (beta  && beta->dtype  == NML_F32) ? beta->data.f32  : NULL;
        if (nml_backend_sycl_layernorm(out->data.f32, n_groups, last_dim, gp, bp) == 0) return;
    }
#endif
#ifdef NML_USE_CUDA
    if (input->dtype == NML_F32 && input->size >= _nml_cuda_ew_thr) {
        const float *gp = (gamma && gamma->dtype == NML_F32) ? gamma->data.f32 : NULL;
        const float *bp = (beta  && beta->dtype  == NML_F32) ? beta->data.f32  : NULL;
        if (nml_backend_cuda_layernorm(out->data.f32, n_groups, last_dim, gp, bp) == 0) return;
    }
#endif
...
#ifdef NML_USE_SYCL
    if (out->dtype == NML_F32 && out->size >= _nml_sycl_ew_thr &&
        nml_backend_sycl_gelu(out->data.f32, out->size) == 0) return;
#endif
#ifdef NML_USE_CUDA
    if (out->dtype == NML_F32 && out->size >= _nml_cuda_ew_thr &&
        nml_backend_cuda_gelu(out->data.f32, out->size) == 0) return;
#endif
    for (int i = 0; i < t->size; i++) {
...
typedef enum {
    ...
    OP_MMUL, OP_MADD, OP_MSUB, OP_EMUL, OP_SDOT, OP_SCLR, OP_SADD, OP_SSUB,
    ...
    /* Extension: NML-TR Phase 3 — batch norm, dropout */
    OP_BN,     /* BN Rd Rs [Rgamma [Rbeta]]  — batch normalisation (2D or 4D) */
    OP_DROP,   /* DROP Rd Rs #p              — inverted dropout with LCG RNG */
#endif
    OP_COUNT
} Opcode;
...
static const OpcodeEntry OPCODE_TABLE[] = {
    ...
    {"SADD", OP_SADD, "core"}, {"SSUB", OP_SSUB, "core"},
    ...
    {"\xe2\x88\x94", OP_SADD, "core"},  /* ∔ */
    {"\xe2\x88\xb8", OP_SSUB, "core"},  /* ∸ */
    /* Activation */
    ...
    {"SCALE",           OP_SCLR, "core"},
    {"SCALAR_ADD",      OP_SADD, "core"},
    {"SCALAR_SUB",      OP_SSUB, "core"},
    {"DIVIDE",          OP_SDIV, "core"},
    ...
    {"JMPNZ",           OP_JMPT, "core"}, /* jump-if-not-zero: alias for JMPT */
    {"JMPZ",            OP_JMPF, "core"}, /* jump-if-zero:     alias for JMPF */
    ...
    {"LOSS_BK",  OP_SOFTBK, "NML-TR"}, /* training alias: loss backward → softmax backward */
    ...
    /* NML-TR Phase 3 — Symbolic */
    {"\xe2\x8a\x9e", OP_BN,    "NML-TR"},  /* ⊞ plus-in-box — batch norm */
    {"\xe2\x89\x8b", OP_DROP,  "NML-TR"},  /* ≋ tilde-equal — dropout */
    /* NML-TR v0.9 — Verbose */
    {"TRAIN_LOG",      OP_TLOG,   "NML-TR"},
    {"TRAIN_CONFIG",   OP_TRAIN,  "NML-TR"},
    {"FORWARD_PASS",   OP_INFER,  "NML-TR"},
    {"WEIGHT_DECAY",   OP_WDECAY, "NML-TR"},
    /* NML-TR Phase 3 — Verbose */
    {"BATCH_NORM",     OP_BN,    "NML-TR"},
    {"DROPOUT",        OP_DROP,  "NML-TR"},
#endif
    {NULL, 0, NULL}
};
...
#endif
#ifdef NML_EXT_TRAINING
    vm->extensions |= EXT_TRAINING;
#endif
}
    ...
    while (*p) {
        ...
        switch (instr->op) {
            ...
            case OP_SCLR: case OP_SDIV:
                ...
                break;
            /* Rd Rs #imm  -or-  Rd Rs1 Rs2: SADD SSUB
             * Third operand may be an immediate or a scalar register.
             * int_params[0] = 0 → immediate mode, 1 → register mode. */
            case OP_SADD: case OP_SSUB:
                instr->reg[0] = parse_register(tokens[1]);
                instr->reg[1] = parse_register(tokens[2]);
                if (is_register_token(tokens[3])) {
                    instr->reg[2]        = parse_register(tokens[3]);
                    instr->int_params[0] = 1; /* scalar from register[0] */
                } else {
                    instr->imm           = parse_imm(tokens[3]);
                    instr->int_params[0] = 0; /* scalar from immediate */
                }
                break;
            ...
            case OP_LOSS:
                ...
                break;
            case OP_TNET:
                /* N-layer config mode: TNET Rcfg #epochs [#lr [#optimizer]]
                 *   When first token is a register → config-driven N-layer TNET.
                 *   reg[0] = config register; imm = epochs; int_params[0] = lr*1e6; int_params[1] = optimizer
                 * Legacy 2-layer mode: TNET #epochs #lr #optimizer
                 *   reg[0] = -1 (no config register)
                 * Long legacy: TNET R0 R1 ... #epochs #lr (registers + immediates)
                 *   Register convention: R0=input, R1=w1, R2=b1, R3=w2, R4=b2, R9=target */
                {
                    /* Check if the very first operand token is a register (and it's the only
                     * register, i.e., not the start of the multi-register legacy syntax).
                     * Heuristic: if tokens[1] is a register AND tokens[2] is '#' or missing,
                     * treat as N-layer config mode; otherwise fall through to legacy logic. */
                    int cfg_mode = 0;
                    if (ntokens > 1 && is_register_token(tokens[1])) {
                        /* If tokens[2] is absent or starts with '#', it's N-layer mode */
                        if (ntokens < 3 || tokens[2][0] == '#') {
                            cfg_mode = 1;
                        }
                    }
                    if (cfg_mode) {
                        instr->reg[0]        = parse_register(tokens[1]);
                        instr->imm           = (ntokens > 2) ? parse_imm(tokens[2]) : 1000;
                        instr->int_params[0] = (ntokens > 3) ? (int)(parse_imm(tokens[3]) * 1e6) : 1000;
                        instr->int_params[1] = (ntokens > 4) ? (int)parse_imm(tokens[4]) : 0;
                    } else {
                        instr->reg[0] = -1; /* legacy mode — no config register */
                        int fi = 1;
            ...
            case OP_WDECAY:
                ...
                break;
            /* BN Rd Rs [Rgamma [Rbeta]]  — batch normalisation
             * reg[0]=Rd, reg[1]=Rs, reg[2]=Rgamma, int_params[3]=Rbeta */
            case OP_BN:
                instr->reg[0] = parse_register(tokens[1]);
                instr->reg[1] = parse_register(tokens[2]);
                instr->reg[2]        = (ntokens > 3) ? parse_register(tokens[3]) : -1;
                instr->int_params[3] = (ntokens > 4) ? parse_register(tokens[4]) : -1;
                break;
            /* DROP Rd Rs #p  — inverted dropout */
            case OP_DROP:
                instr->reg[0] = parse_register(tokens[1]);
                instr->reg[1] = parse_register(tokens[2]);
                instr->imm    = (ntokens > 3) ? parse_imm(tokens[3]) : 0.5;
                break;
#endif
            default:
...
static int vm_resolve_fragments(VM *vm, const char *entry_fragment) {
    ...
    /* Heap-allocate to avoid stack overflow (each Instruction is ~272 bytes) */
    Instruction *expanded = (Instruction *)malloc(NML_MAX_INSTRUCTIONS * sizeof(Instruction));
    if (!expanded) return NML_ERR_MEMORY;
    int exp_len = 0;
    ...
    WorkItem *work = (WorkItem *)malloc(NML_MAX_INSTRUCTIONS * sizeof(WorkItem));
    if (!work) { free(expanded); return NML_ERR_MEMORY; }
    int work_len = 0;
    ...
    while (wi < work_len && exp_len < NML_MAX_INSTRUCTIONS) {
        ...
        if (ins->op == OP_LINK) {
            ...
            if (found < 0) {
                ...
                free(expanded); free(work); return NML_ERR_ASSEMBLE;
            }
...
static int vm_execute(VM *vm) {
    ...
    while (vm->pc < vm->program_len && !vm->halted) {
        if (vm->max_cycles > 0 && vm->cycles >= vm->max_cycles)
        ...
        switch (ins->op) {
            case OP_MMUL: rc=tensor_matmul(&REG(ins->reg[0]),&REG(ins->reg[1]),&REG(ins->reg[2])); if(rc) VM_ERROR(vm,rc,"MMUL shape error at PC=%d",vm->pc); RVALID(ins->reg[0])=1;
                ...
                if (vm->pc + 1 < vm->program_len) {
                    Instruction *next_ins = &vm->program[vm->pc + 1];
                    int fuse_reg = ins->reg[0];
                    if ((next_ins->op == OP_RELU || next_ins->op == OP_SIGM ||
                         next_ins->op == OP_TANH
#ifdef NML_EXT_TRANSFORMER
                         || next_ins->op == OP_GELU
#endif
                         ) &&
                        next_ins->reg[1] == fuse_reg && next_ins->reg[0] == fuse_reg) {
                        switch (next_ins->op) {
                            case OP_RELU: tensor_relu(&REG(fuse_reg), &REG(fuse_reg)); break;
                            case OP_SIGM: tensor_sigmoid(&REG(fuse_reg), &REG(fuse_reg)); break;
                            case OP_TANH: tensor_tanh_act(&REG(fuse_reg), &REG(fuse_reg)); break;
#ifdef NML_EXT_TRANSFORMER
                            case OP_GELU: tensor_gelu(&REG(fuse_reg), &REG(fuse_reg)); break;
#endif
                            default: break;
                        }
                        vm->pc++;
            ...
            case OP_SADD: { double s=ins->int_params[0]?tensor_getd(&REG(ins->reg[2]),0):ins->imm; tensor_scale_add(&REG(ins->reg[0]),&REG(ins->reg[1]),s); RVALID(ins->reg[0])=1; break; }
            case OP_SSUB: { double s=ins->int_params[0]?tensor_getd(&REG(ins->reg[2]),0):ins->imm; tensor_scale_sub(&REG(ins->reg[0]),&REG(ins->reg[1]),s); RVALID(ins->reg[0])=1; break; }
            ...
            /* ═══ NML-V Vision ═══ */
#ifdef NML_EXT_VISION
            case OP_CONV: CHECK_EXT(EXT_VISION,"NML-V"); tensor_conv2d(&REG(ins->reg[0]),&REG(ins->reg[1]),&REG(ins->reg[2]),ins->int_params[0],ins->int_params[1]); RVALID(ins->reg[0])=1;
                /* Lookahead fusion: if next op is an activation on the same output register, fuse in-place */
                if (vm->pc + 1 < vm->program_len) {
                    Instruction *next_ins = &vm->program[vm->pc + 1];
                    int fuse_reg = ins->reg[0];
        ...
        case OP_LOSS: {
            /* LOSS Rd Rs_pred Rs_target #type (0=MSE, 1=MAE, 2=CE) */
            Tensor *pred = &REG(ins->reg[1]);
            Tensor *target = &REG(ins->reg[2]);
            int loss_type = ins->int_params[0];
            if (loss_type == 2) {
                /* Categorical cross-entropy with internal log-softmax */
                int n = REG(ins->reg[1]).size;
                /* Find max for numerical stability */
                double max_v = tensor_getd(&REG(ins->reg[1]), 0);
                for (int i = 1; i < n; i++) {
                    double v = tensor_getd(&REG(ins->reg[1]), i);
                    if (v > max_v) max_v = v;
                }
                /* log-sum-exp */
                double lse = 0.0;
                for (int i = 0; i < n; i++)
                    lse += exp(tensor_getd(&REG(ins->reg[1]), i) - max_v);
                lse = log(lse) + max_v;
                /* CE = -sum(target * log_prob) */
                double ce = 0.0;
                for (int i = 0; i < n; i++) {
                    double t = tensor_getd(&REG(ins->reg[2]), i);
                    if (t > 0.0)
                        ce -= t * (tensor_getd(&REG(ins->reg[1]), i) - lse);
                }
                int s[] = {1};
                tensor_init_typed(&REG(ins->reg[0]), 1, s, NML_F64);
                tensor_setd(&REG(ins->reg[0]), 0, ce);
                RVALID(ins->reg[0]) = 1;
            } else {
        ...
        case OP_TNET: {
            ...
            if (ins->reg[0] >= 0) {
                ...
                int L = cfg->shape[0];
                if (L < 1 || L > 8) VM_ERROR(vm, NML_ERR_OPCODE, "TNET: config layers must be 1-8, got %d", L);

                int epochs_n  = (int)ins->imm;
                ...
                for (int l = 0; l < L; l++) {
                    ...
                    out_sz[l]  = (int)tensor_getd(cfg, l*3 + 1);
        ...
        /* ═══ NML-TR Phase 3: Batch Normalisation ═══ */
        case OP_BN: {
            /* BN Rd Rs [Rgamma [Rbeta]]
             * Rs: input [N, C] or [N, C, H, W]
             * Rd: normalised output (same shape)
             * Rgamma (optional): scale [C], Rbeta (optional): shift [C] */
            Tensor *inp = &REG(ins->reg[1]);
            if (!RVALID(ins->reg[1]))
                VM_ERROR(vm, NML_ERR_OPCODE, "BN: source register R%d is empty", ins->reg[1]);
            Tensor *gamma_bn = (ins->reg[2] >= 0 && RVALID(ins->reg[2])) ? &REG(ins->reg[2]) : NULL;
            Tensor *beta_bn  = (ins->int_params[3] >= 0 && RVALID(ins->int_params[3])) ? &REG(ins->int_params[3]) : NULL;

            int N_bn, C_bn, spatial_bn = 1;
            if (inp->ndim == 2) {
                N_bn = inp->shape[0]; C_bn = inp->shape[1];
            } else if (inp->ndim == 4) {
                N_bn = inp->shape[0]; C_bn = inp->shape[1];
                spatial_bn = inp->shape[2] * inp->shape[3];
            } else {
                VM_ERROR(vm, NML_ERR_SHAPE, "BN: expected 2D or 4D input, got %dD", inp->ndim);
            }

            tensor_init_typed(&REG(ins->reg[0]), inp->ndim, inp->shape, NML_F32);

            float eps_bn = 1e-5f;
            for (int c = 0; c < C_bn; c++) {
                int count_bn = N_bn * spatial_bn;
                /* Mean */
                double mean_bn = 0.0;
                for (int n = 0; n < N_bn; n++)
                    for (int s = 0; s < spatial_bn; s++) {
                        int idx = (inp->ndim == 2) ? n*C_bn+c : n*(C_bn*spatial_bn)+c*spatial_bn+s;
                        mean_bn += tensor_getd(inp, idx);
                    }
                mean_bn /= count_bn;
                /* Variance (use std dev directly) */
                double var_bn = 0.0;
                for (int n = 0; n < N_bn; n++)
                    for (int s = 0; s < spatial_bn; s++) {
                        int idx = (inp->ndim == 2) ? n*C_bn+c : n*(C_bn*spatial_bn)+c*spatial_bn+s;
                        double d = tensor_getd(inp, idx) - mean_bn;
                        var_bn += d * d;
                    }
                double std_bn = sqrt(var_bn / count_bn + eps_bn);
                /* Normalize and apply gamma/beta */
                double g_bn = (gamma_bn && (int)gamma_bn->size > c) ? tensor_getd(gamma_bn, c) : 1.0;
                double bv   = (beta_bn  && (int)beta_bn->size  > c) ? tensor_getd(beta_bn,  c) : 0.0;
                for (int n = 0; n < N_bn; n++)
                    for (int s = 0; s < spatial_bn; s++) {
                        int idx = (inp->ndim == 2) ? n*C_bn+c : n*(C_bn*spatial_bn)+c*spatial_bn+s;
                        double norm_val = g_bn * (tensor_getd(inp, idx) - mean_bn) / std_bn + bv;
                        tensor_setd(&REG(ins->reg[0]), idx, norm_val);
                    }
            }
            RVALID(ins->reg[0]) = 1;
            break;
        ...
        /* ═══ NML-TR Phase 3: Dropout ═══ */
        case OP_DROP: {
            /* DROP Rd Rs #p  — inverted dropout
             * p: drop probability [0,1). Elements dropped with prob p; kept scaled by 1/(1-p). */
            float p_drop = (float)ins->imm;
            Tensor *inp_drop = &REG(ins->reg[1]);
            if (!RVALID(ins->reg[1]))
                VM_ERROR(vm, NML_ERR_OPCODE, "DROP: source register R%d is empty", ins->reg[1]);
            tensor_init_typed(&REG(ins->reg[0]), inp_drop->ndim, inp_drop->shape, inp_drop->dtype);

            /* LCG RNG — fast, deterministic, no stdlib rand dependency */
            static unsigned int _drop_seed = 0x12345678u;
            float scale_drop = (p_drop < 1.0f) ? 1.0f / (1.0f - p_drop) : 0.0f;
            for (int i = 0; i < (int)inp_drop->size; i++) {
                _drop_seed = _drop_seed * 1664525u + 1013904223u;
                float r = (_drop_seed >> 8) / 16777216.0f; /* [0,1) */
                double val_drop = (r >= p_drop) ? tensor_getd(inp_drop, i) * scale_drop : 0.0;
                tensor_setd(&REG(ins->reg[0]), i, val_drop);
            }
            RVALID(ins->reg[0]) = 1;
            break;
        }
...
static int vm_load_data(VM *vm, const char *path) {
    ...
    while (*p) {
        ...
        if (*start == '@') {
            ...
            while (*lp && *lp != ' ' && *lp != '\t' && li < NML_MAX_LABEL_LEN - 1) label[li++] = *lp++;
            char *sp = strstr(lp, "shape=");
            if (sp) { sp += 6; while (*sp && *sp != ' ' && *sp != '\t' && ndim < NML_MAX_DIMS) { shape[ndim++] = atoi(sp); while (*sp && *sp != ',' && *sp != ' ' && *sp != '\t') sp++; if (*sp == ',') sp++; } }
...
int main(int argc, char **argv) {
    ...
    if (!program_path) {
        ...
        printf("v0.7 additions: BKWD WUPD LOSS TNET (NML-TR training); extended registers RG-RV\n");
        printf("v0.8 additions: RELUBK SIGMBK TANHBK GELUBK SOFTBK MMULBK CONVBK POOLBK NORMBK ATTNBK TNDEEP\n");
        printf("  + backward passes for all activations, MMUL, CONV, POOL, NORM, ATTN\n");
        printf("v0.9 additions: TLOG TRAIN INFER WDECAY (config-driven training + AdamW)\n");
        printf("  + CONV/POOL/UPSC/PADZ 4D NCHW; broadcasting (MADD/MSUB/EMUL/EDIV); LOSS #2 cross-entropy\n");
        printf("  + CUDA backend (cuBLAS GEMM, CONV im2col, ATTN, GELU, SOFTMAX, LAYERNORM, fusion)\n");
        printf("  + SYCL backend (Intel oneAPI, same op set as CUDA)\n");
        printf("v0.10 additions: BN (batch norm) DROP (inverted dropout); N-layer TNET (1-8 layers)\n");
        printf("  + ONNX import (nml_from_onnx.py, 28 op types); Python binding (ctypes + subprocess)\n");
        printf("  + C library API (nml_api.h): nml_vm_create/load/set_input/run/get_output/destroy\n");
        return 1;
    }
    ...
    if (vm->extensions & EXT_TRAINING) printf(" NML-TR");
...
#ifdef NML_USE_HAILO
    /* Hailo NPU dispatch — two modes selected at compile time:
     *
     * NML_EMBEDDED_HEF (make nml-rpi-hailo-embed):
     *   HEF is a C byte-array in nml_hef_resource.h, compiled into this binary.
     *   No file I/O.  Single self-contained binary for deployment.
     *
     * Default (make nml-rpi-hailo):
     *   HEF lookup on disk, priority order:
     *     1. $HAILO_ARCH env var  → <stem>.<env_arch>.hef
     *     2. Auto-detected arch   → <stem>.<dev_arch>.hef
     *     3. Generic fallback     → <stem>.hef
     *   Falls back to vm_execute() if no HEF found or dispatch fails.        */
    {
        int  hailo_ok = 0;
        char hailo_err[256] = {0};

        /* ── Build the input slot array (shared by both modes) ── */
        NMLHailoTensor inputs[NML_MAX_MEMORY_SLOTS];
        int n_in = 0;
        for (int i = 0; i < vm->mem_count; i++) {
            if (vm->memory[i].used && vm->memory[i].tensor.dtype == NML_F32) {
                strncpy(inputs[n_in].name, vm->memory[i].label,
                        sizeof(inputs[n_in].name) - 1);
                inputs[n_in].name[sizeof(inputs[n_in].name) - 1] = '\0';
                inputs[n_in].data = vm->memory[i].tensor.data.f32;
                inputs[n_in].n    = vm->memory[i].tensor.size;
                n_in++;
            }
        }
        NMLHailoTensor outputs[16];
        int n_out = 0;
        int hrc   = -1;

#ifdef NML_EMBEDDED_HEF
        /* ── Embedded mode: HEF is a byte array in this binary ── */
        printf("[NML] Hailo embedded HEF (%u bytes) — dispatching to NPU\n",
               _nml_embedded_hef_size);
        hrc = nml_hailo_run_mem(_nml_embedded_hef_data, _nml_embedded_hef_size,
                                inputs, n_in, outputs, 16, &n_out,
                                hailo_err, sizeof(hailo_err));
        hailo_ok = (hrc == 0);
        if (!hailo_ok)
            fprintf(stderr, "[NML] Hailo embedded dispatch failed (%s) — falling back to CPU\n",
                    hailo_err[0] ? hailo_err : "unknown error");
#else
        /* ── File mode: locate chip-specific or generic sidecar HEF ── */
        {
            char hef_stem[504];
            char hef_path[512];
            strncpy(hef_stem, program_path, sizeof(hef_stem) - 1);
            hef_stem[sizeof(hef_stem) - 1] = '\0';
            char *dot = strrchr(hef_stem, '.');
            if (dot) *dot = '\0';

            const char *env_arch  = getenv("HAILO_ARCH");
            const char *dev_arch  = nml_hailo_arch();
            int arch_known = dev_arch && strcmp(dev_arch, "unknown") != 0;

            int hef_found = 0;
            if (!hef_found && env_arch && env_arch[0]) {
                snprintf(hef_path, sizeof(hef_path), "%s.%s.hef", hef_stem, env_arch);
                FILE *f = fopen(hef_path, "rb"); if (f) { fclose(f); hef_found = 1; }
            }
            if (!hef_found && arch_known) {
                snprintf(hef_path, sizeof(hef_path), "%s.%s.hef", hef_stem, dev_arch);
                FILE *f = fopen(hef_path, "rb"); if (f) { fclose(f); hef_found = 1; }
            }
            if (!hef_found) {
                snprintf(hef_path, sizeof(hef_path), "%s.hef", hef_stem);
                FILE *f = fopen(hef_path, "rb"); if (f) { fclose(f); hef_found = 1; }
            }

            if (hef_found) {
                printf("[NML] Hailo HEF: %s (arch: %s) — dispatching to NPU\n",
                       hef_path,
                       arch_known ? dev_arch : (env_arch ? env_arch : "generic"));
                hrc = nml_hailo_run(hef_path,
                                    inputs, n_in, outputs, 16, &n_out,
                                    hailo_err, sizeof(hailo_err));
                hailo_ok = (hrc == 0);
                if (!hailo_ok)
                    fprintf(stderr, "[NML] Hailo dispatch failed (%s) — falling back to CPU\n",
                            hailo_err[0] ? hailo_err : "unknown error");
            }
        }
#endif /* NML_EMBEDDED_HEF */

        if (hailo_ok) {
            /* Store NPU outputs into vm->memory so register dump + --output see them */
            for (int i = 0; i < n_out; i++) {
                MemorySlot *slot = NULL;
                for (int j = 0; j < vm->mem_count; j++) {
                    if (strcmp(vm->memory[j].label, outputs[i].name) == 0) {
                        slot = &vm->memory[j]; break;
                    }
                }
                if (!slot && vm->mem_count < NML_MAX_MEMORY_SLOTS) {
                    slot = &vm->memory[vm->mem_count++];
                    strncpy(slot->label, outputs[i].name, NML_MAX_LABEL_LEN - 1);
                }
                if (slot) {
                    tensor_free(&slot->tensor);
                    int shape[1] = { outputs[i].n };
                    tensor_init_typed(&slot->tensor, 1, shape, NML_F32);
                    memcpy(slot->tensor.data.f32, outputs[i].data,
                           (size_t)outputs[i].n * sizeof(float));
                    slot->used = 1;
                }
                free(outputs[i].data);
            }
            printf("[NML] Hailo inference complete (%d output tensors)\n", n_out);
            result = NML_OK;
        }
...
/* ════════════════════════════════════════════════════════════════════════════
   PUBLIC C API  (nml_api.h)
   Compiled unconditionally — available in both the CLI binary and the library.
   When building the shared/static library, define NML_BUILD_LIB (which also
   implies NML_LIBRARY_MODE so that main() is omitted).
   ════════════════════════════════════════════════════════════════════════════ */

#ifdef NML_BUILD_LIB
  /* Ensure main() is excluded when building as a library */
  #ifndef NML_LIBRARY_MODE
...
/* ── Version string ──────────────────────────────────────────────────────── */
static char _nml_version_buf[16];

const char *nml_version(void) {
    if (_nml_version_buf[0] == '\0') {
        snprintf(_nml_version_buf, sizeof(_nml_version_buf),
                 "%d.%d.0", NML_VERSION_MAJOR, NML_VERSION_MINOR);
    }
    return _nml_version_buf;
}

/* ── Opcode count ────────────────────────────────────────────────────────── */
int nml_opcode_count(void) {
    /* OPCODE_TABLE is sentinel-terminated with .name == NULL */
    int n = 0;
    while (OPCODE_TABLE[n].name) n++;
    return n;
}
...
void nml_vm_destroy(nml_vm_t *handle) {
    if (!handle) return;
    VM *vm = (VM *)handle;
    vm_cleanup(vm);
    free(vm);
...
int nml_vm_load(nml_vm_t *handle, const char *program_path, const char *data_path) {
    if (!handle || !program_path) return NML_ERR_FILE;
    VM *vm = (VM *)handle;

    /* Load data file first (weights must be in memory before assembly) */
    if (data_path) {
        int n = vm_load_data(vm, data_path);
        if (n < 0) {
            snprintf(vm->error_msg, sizeof(vm->error_msg),
                     "Failed to load data file: %s (code %d)", data_path, n);
            return n;
        }
    }

    /* Read and assemble program source */
    char *source = read_file(program_path);
    if (!source) {
        snprintf(vm->error_msg, sizeof(vm->error_msg),
                 "Cannot open program file: %s", program_path);
        return NML_ERR_FILE;
    }
    ...
    free(source);
    if (ninstr < 0) {
        /* error_msg already set by vm_assemble */
        return ninstr;
    }

#ifndef NML_NO_M2M
    int frc = vm_resolve_fragments(vm, NULL);
    if (frc != NML_OK) return frc;
#endif
...
int nml_vm_load_mem(nml_vm_t *handle, const char *program_text, const char *data_text) {
    if (!handle || !program_text) return NML_ERR_FILE;
    VM *vm = (VM *)handle;

    if (data_text) {
        int n = vm_load_data_from_string(vm, data_text);
        if (n < 0) {
            snprintf(vm->error_msg, sizeof(vm->error_msg),
                     "Failed to parse data text (code %d)", n);
            return n;
        }
    }

    int ninstr = vm_assemble(vm, program_text);
    if (ninstr < 0) return ninstr;
...
int nml_vm_set_input(nml_vm_t *handle, const char *name,
                     const float *data, const int *shape, int shape_len) {
    if (!handle || !name || !data || !shape || shape_len < 1 || shape_len > NML_MAX_DIMS)
        return NML_ERR_OOB;
    VM *vm = (VM *)handle;

    MemorySlot *slot = vm_memory(vm, name);
    if (!slot) return NML_ERR_MEMORY;

    tensor_free(&slot->tensor);
    int rc = tensor_init_typed(&slot->tensor, shape_len, shape, NML_F32);
    if (rc != 0) return rc;

    memcpy(slot->tensor.data.f32, data, sizeof(float) * (size_t)slot->tensor.size);
    slot->used = 1;
    return NML_OK;
}
...
int nml_vm_run(nml_vm_t *handle) {
    if (!handle) return NML_ERR_OOB;
    VM *vm = (VM *)handle;
    /* Reset execution state but keep program and memory intact */
    vm->pc        = 0;
    vm->halted    = 0;
    vm->cycles    = 0;
    vm->cond_flag = 0;
...
NMLTensor nml_vm_get_output(nml_vm_t *handle, const char *name) {
    NMLTensor result;
    result.data = NULL;
    result.ndim = 0;
    result.size = 0;
    result.shape[0] = result.shape[1] = result.shape[2] = result.shape[3] = 0;

    if (!handle || !name) return result;
    VM *vm = (VM *)handle;

    for (int i = 0; i < vm->mem_count; i++) {
        if (strcmp(vm->memory[i].label, name) == 0 && vm->memory[i].used) {
            Tensor *t = &vm->memory[i].tensor;
            if (t->dtype != NML_F32 || !t->data.f32) return result;
            result.data = t->data.f32;
            result.ndim = t->ndim;
            result.size = t->size;
            for (int d = 0; d < t->ndim && d < 4; d++)
                result.shape[d] = t->shape[d];
            return result;
        }
    }
    return result;
...
NMLTensor nml_vm_get_register(nml_vm_t *handle, int reg_index) {
    NMLTensor result;
    result.data = NULL;
    result.ndim = 0;
    result.size = 0;
    ...
    if (!handle || reg_index < 0 || reg_index >= NML_MAX_REGISTERS) return result;
    VM *vm = (VM *)handle;
    if (!vm->reg_valid[reg_index]) return result;

    Tensor *t = &vm->regs[reg_index];
    if (t->dtype != NML_F32 || !t->data.f32) return result;
...
const char *nml_vm_last_error(nml_vm_t *handle) {
    if (handle) {
        VM *vm = (VM *)handle;
        if (vm->error_msg[0]) return vm->error_msg;
    }
    return "no error";
}
