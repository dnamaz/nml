...
// ── Server config ─────────────────────────────────────────────────────────────
// Think model (Qwen3.5-4B reasoning) on port 8084
// Code model  (nml-v09-merged-6bit)  on port 8082
const THINK_BASE = "http://localhost:8084/v1";
const CODE_BASE  = "http://localhost:8082/v1";

const THINK_SYSTEM = `You are an NML (Neural Machine Language) architecture planner.
...
const NML_OPCODES = new Set([
  ...
  "BKWD","WUPD","LOSS","TNET","TNDEEP","TRAIN","INFER","WDECAY","TLOG",
...
@target   shape=1,3   dtype=f32  data=0.0,1.0,0.0`;
  ...
  const runExecute = async (code) => {
    ...
    setExecOut("");
    try {
      const r = await fetch("http://localhost:8082/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: "#8b949e" }}>Think model → Code model → Execute</div>
          </div>
          <div style={{ fontSize: 11, color: "#30363d" }}>Think :8084 &nbsp;|&nbsp; Code :8082</div>
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, display: "flex", gap: 12, padding: "12px 16px 6px", overflow: "hidden" }}>
            ...
            <Panel title="Code Model" subtitle="nml-v09-merged-6bit · NML Assembly"
