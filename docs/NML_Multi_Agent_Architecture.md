...
| Symbolic Engine | `runtime/nml.c` — 89-opcode VM, ~83KB | Unchanged. The engine is already deterministic and correct. No LLM needed. |
...
| Implementation | `runtime/nml.c` — 89 opcodes, ~83KB binary |
...
This separation is what makes the system trustworthy in regulated domains. You do not need to trust the LLM. You verify the NML it produces. And you can verify NML because it is an 89-opcode formal language, not a 50,000-token natural language.
...
## Part 8: Vision and CNN Support

### 4D NCHW Vision Ops in the Agent Pipeline

Phase 1 extended NML's vision ops (CONV, POOL, UPSC, PADZ) to accept 4D tensors in standard NCHW layout:

- Input: `[N, C_in, H, W]`
- Kernel: `[C_out, C_in, KH, KW]`
- Output: `[N, C_out, H_out, W_out]`

This closes the gap between NML and ONNX-exported CNNs. A ResNet, MobileNet, or EfficientNet exported from PyTorch produces NCHW-layout convolutions by default. Before Phase 1, those shapes were incompatible with NML's 2D CONV. Now they map directly.

### How ONNX Import Enables Pre-Trained Vision Deployment

Phase 4 added `transpilers/nml_from_onnx.py`, which converts an ONNX model into an NML program and a `.nml.data` weight file. Combined with Phase 1's 4D vision ops, the full pipeline is:

```
Pre-trained PyTorch/TF model
        │
        ▼  torch.onnx.export() / tf2onnx
  model.onnx
        │
        ▼  python3 transpilers/nml_from_onnx.py model.onnx
  model.nml + model.nml.data
        │
        ▼  ./nml model.nml model.nml.data   (or libnml / python binding)
  inference result
```

The transpiler handles the register allocation automatically: primary input goes to R0, intermediate activations to R1–R28, and all weight tensors (initializers) become named memory slots (`@layer1_weight`, etc.). No manual porting is needed.

### Integration with the Multi-Agent Pipeline

In the multi-agent architecture (Parts 1–7), the NML runtime acts as the deterministic symbolic engine. With vision support, that engine now handles:

1. **Edge vision agents**: A camera node on a Raspberry Pi + Hailo NPU runs a MobileNet converted via `nml_from_onnx.py → nml_to_hailo.py`. The NML program is the interchange format — the same `.nml` file that runs on the development machine's CPU runs on the edge NPU. No separate edge build.

2. **Multi-agent visual consensus**: Multiple agents independently run the same CNN on the same image data. `VOTE R1 R0 #0` (median consensus) across 5 agent outputs eliminates single-agent outliers — the same `VOTE` instruction that works for scalar tax calculations works for vision classification scores.

3. **ONNX import as agent capability**: An orchestrator agent can call `nml_from_onnx.py` as a service: POST an ONNX model, receive a signed NML program. The signed program (`SIGN`/`VRFY`) proves the model was transpiled by a trusted agent — not injected or tampered with in transit. This extends the M2M trust protocol (Part 5) to include model provenance.

4. **C API embedding in agent processes**: Each agent in the pipeline can embed NML inference directly via `libnml.so` (Phase 5) rather than spawning a subprocess per request. A Python-based agent uses `nml.VM()` from `python/nml.py`; a Go or Rust agent links against `libnml.a`. The opaque `nml_vm_t*` handle means multiple agents in the same process can each hold their own VM without interference.

### Hailo NPU: Whole-Model Vision Dispatch

For Raspberry Pi deployments with the AI HAT or AI HAT+ 2, the ONNX → Hailo path is:

```bash
# Step 1: transpile to NML (CPU reference, validates correctness)
python3 transpilers/nml_from_onnx.py model.onnx

# Step 2: compile to Hailo HEF (requires Hailo Dataflow Compiler, dev machine)
python3 transpilers/nml_to_hailo.py model.nml model.nml.data --arch hailo8l

# Step 3: deploy (Raspberry Pi, no DFC needed)
./nml-rpi-hailo model.nml model.nml.data
# → Hailo NPU handles inference; NML interpreter never runs
```

The NML file serves two roles: reference executable (CPU) and input spec for the HEF compiler. The same program is used for local validation on the dev machine and for production dispatch on the NPU. `nml_to_hailo.py` was updated in Phase 4 to correctly handle CONV4D, which was the main blocker for real CNN deployment.

### Shape and Constraint Summary

| Op | Input shape | Kernel/pool shape | Output shape |
|----|-------------|-------------------|--------------|
| CONV | `[N,C_in,H,W]` | `[C_out,C_in,KH,KW]` | `[N,C_out,H_out,W_out]` |
| POOL | `[N,C,H,W]` | `#KH #KW #stride` | `[N,C,H_out,W_out]` |
| UPSC | `[N,C,H,W]` | `#scale` | `[N,C,H*scale,W*scale]` |
| PADZ | `[N,C,H,W]` | `#pad` | `[N,C,H+2*pad,W+2*pad]` |

2D inputs continue to work — the ops branch on `ndim`. Mixed-dimensionality programs (2D MLP layers + 4D CNN layers in the same program) are supported.
...
| **NML** | Neural Machine Language — an 89-opcode formal language for AI workloads |
